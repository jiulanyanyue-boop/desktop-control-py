...
if defined UV_EXE (
  if defined PYTHON_EXE (
    "%UV_EXE%" run --python "%PYTHON_EXE%" --project "%PROJECT_ROOT%" desktop-control-py serve --transport stdio
  ) else (
    "%UV_EXE%" run --project "%PROJECT_ROOT%" desktop-control-py serve --transport stdio
  )
  exit /b %errorlevel%
)

if defined PYTHON_EXE (
  "%PYTHON_EXE%" -m desktop_control_py serve --transport stdio
  exit /b %errorlevel%
)
