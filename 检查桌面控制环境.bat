...
echo [desktop-control-py] Checking Python MCP runtime...
if defined UV_EXE (
  if defined PYTHON_EXE (
    "%UV_EXE%" run --python "%PYTHON_EXE%" --project "%PROJECT_ROOT%" desktop-control-py check-env
  ) else (
    "%UV_EXE%" run --project "%PROJECT_ROOT%" desktop-control-py check-env
  )
  exit /b %errorlevel%
)

if defined PYTHON_EXE (
  "%PYTHON_EXE%" -m desktop_control_py check-env
  exit /b %errorlevel%
)
